...
#########################################################################
def about():
    messagebox.showinfo("About", "Notepad by Code With Bhumi")


def help_box(event=None):
    messagebox.showinfo("Help", "For help email to rockyjagtiani@gmail.com", icon='question')
...
def new_file(event=None):
    ...
    filename = None
    root.title("Untitled - notepad")
    textPad.delete(1.0, END) ### first line with zero char
    update_line_number()
...
def open_file(event=None):
    ...
    if filename == "":  # If no file chosen.
        ...
        root.title(os.path.basename(filename) + " - notepad")  # Returning the basename of 'file'
        textPad.delete(1.0, END)
        fh = open(filename, "r")   # open file in read mode
        textPad.insert(1.0, fh.read())  # work like delete in which inser from 1 line of zero char
        fh.close()
    update_line_number()


def save(event=None):
    global filename # create file as global var
    try:
        f = open(filename, 'w')
        letter = textPad.get(1.0, 'end')  # use to write file
        f.write(letter)
        f.close()
    except:
        save_as()
...
def save_as():
    try:
        ...
        fh.write(textoutput)
        fh.close()
        root.title(os.path.basename(f) + " - notepad")  # Setting the title of the root widget.
    except:
        pass
